...
KNOB<bool> DOPCOMM(KNOB_MODE_WRITEONCE, "pintool", "cc", "0", "enable com detection (paired with timestamp)");
...
VOID add_comm_event(int a, int b, UINT64 tsc, UINT32 msize) {
    if (a != b - 1)
        update_comm_events(a, b, tsc, msize);
}
...
VOID do_pair_comm(ADDRINT addr, THREADID tid, UINT32 dsize) {
    ...
    UINT64 curr_ts = get_tsc() / CYCLE_RESOLUTION;
    UINT64 line = addr >> COMMSIZE;
    tid = real_tid(tid);
    //int sh = 1;
