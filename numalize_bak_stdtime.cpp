...
KNOB<bool> DOCOMMLOAD(KNOB_MODE_WRITEONCE, "pintool", "cl", "0", "enable comm (with load) detection");
KNOB<bool> DOPAGE(KNOB_MODE_WRITEONCE, "pintool", "p", "0", "enable page usage detection");
KNOB<bool> DOPCOMM(KNOB_MODE_WRITEONCE, "pintool", "cc", "0", "enable com detection (paired)");
...
UINT64 comm_size_matrix[MAXTHREADS][MAXTHREADS];

// communication events
//std::vector<CommEvent> commEvents;
//std::map<UINT64, std::vector<CommEvent>> eventMap;

//std::vector<CommEvent> pairEvents [MAXTHREADS][MAXTHREADS];
//void *db;
//TimedEventArr pairEvents [MAXTHREADS][MAXTHREADS];
class TPayload {
public:
    UINT32 count;
    UINT64 size; 
...
unordered_map<std::time_t, TPayload> timeEventMap [MAXTHREADS][MAXTHREADS];
...
VOID inc_comm_load(int a, int b, UINT32 dsize) {
     if (a != b - 1) {
        comm_matrix[a][b - 1]++;
        comm_size_matrix[a][b -1] += dsize;
     }
}

static inline
VOID update_comm_events(int a, int b, std::time_t sec, UINT32 msize) {
    timeEventMap[a][b - 1][sec].size = timeEventMap[a][b - 1][sec].size + msize;
    timeEventMap[a][b - 1][sec].count = timeEventMap[a][b - 1][sec].count + 1;
}

static inline
VOID add_comm_event(int a, int b, std::time_t sec, UINT32 msize) {
    if (a != b - 1)
        update_comm_events(a, b, sec, msize);
}
...
VOID print_comm_events() {
    ofstream f, f_accu;
    //string fname = "comm_events.out"; 
    char fname[255];
    char fname_accu[255];
    //for (CommEvent e: commEvents) {
    //   f << e.src << "," << e.dest << "," << e.msize << "," << e.tstamp << '\n';
    //}
    int cs = COMMSIZE;
...
int main(int argc, char *argv[]) {
    ...
    if (!DOCOMM && !DOPAGE && !DOPCOMM && !DOCOMMLOAD) {
