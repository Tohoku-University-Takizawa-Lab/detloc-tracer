# PIN_ROOT must point to the Pin kit root.
PIN_ROOT=${HOME}/pin-3.7

# Specify Pin path manually:
# override PIN_ROOT = /path/to/pin

###################################
### Do not edit below this line ###
###################################

# Try to detect Pin installation directory in user's /home and /opt
ifeq ($(PIN_ROOT), )
  override PIN_ROOT = $(shell cat .pin_root 2>/dev/null)
  ifeq ($(PIN_ROOT), )
    override PIN_ROOT = $(shell dirname $$(find ~ /opt -type f -name pin -path '*/*pin' -print -quit))
  endif
  ifeq ($(PIN_ROOT), )
    $(error Could not detect Pin installation directory, please specify PIN_ROOT manually in the Makefile)
  endif
endif

# Store path to Pin if in top-level make
ifeq ($(MAKECMDGOALS), )
  $(info =======================================)
  $(info Using Pin installation in ${PIN_ROOT})
  $(info =======================================)
  $(shell echo ${PIN_ROOT} > .pin_root)
endif

ifdef PIN_ROOT
CONFIG_ROOT := $(PIN_ROOT)/source/tools/Config
else
CONFIG_ROOT := ../Config
endif
include $(CONFIG_ROOT)/makefile.config

#TOOL_CXXFLAGS += -Wall -g -std=c++0x -Wno-error -Wextra -Wno-unused-parameter -pedantic -lelf
TOOL_CXXFLAGS += -Wall -g -std=c++11 -Wno-error -Wextra -Wno-unused-parameter -pedantic -lelf -fomit-frame-pointer
#TOOL_CXXFLAGS += -Wall -g -std=c++11 -Wno-error -Wextra -Wno-unused-parameter -pedantic -lelf 
#-fomit-frame-pointer
# TOOL_LIBS += -lelf
#TOOL_LIBS += /opt/pin-2.14-67254-gcc.4.4.7-linux/intel64/lib-ext/libelf.a
#TOOL_LIBS += $(PIN_ROOT)/intel64//lib-ext/libpin3dwarf.so
#TOOL_LIBS += $(PIN_ROOT)/intel64//lib-ext/libelf.a
#TOOL_LIBS += /usr/lib/x86_64-linux-gnu/libelf.a
TOOL_LIBS += /usr/lib64/libelf.a
#TOOL_LIBS += /shome/agung/local/lib/libelf.a
# Compile libelf (0.8.13) from this http://www.mr511.de/software/english.html
TOOL_ROOTS := detloc_tracer

include $(TOOLS_ROOT)/Config/makefile.default.rules
